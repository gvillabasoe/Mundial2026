...
const STRONG_TEAMS = [
  "España", "Brasil", "Argentina", "Francia", "Alemania", "Inglaterra",
  "Portugal", "Países Bajos", "Bélgica", "Croacia", "Uruguay", "Colombia",
  "Japón", "Estados Unidos", "México", "Marruecos", "Senegal", "Ecuador",
  "Suiza", "Turquía", "Australia", "Austria", "Corea del Sur", "Canadá",
  "Costa de Marfil", "Noruega", "Ghana", "Paraguay", "Escocia", "Egipto",
  "Irán", "Argelia",
];
...
function generateMockParticipants(): Team[] {
  ...
  for (const u of MOCK_USERS_SEED) {
    for (const teamName of u.teamNames) {
      ...
      // Group order picks: shuffle each group
      const groupOrderPicks: Record<string, string[]> = {};
      for (const [g, gTeams] of Object.entries(GROUPS)) {
        groupOrderPicks[g] = shuffle([...gTeams], rand);
      }

      // Knockout picks (cascading pool from STRONG_TEAMS)
      const d16pool = shuffle([...STRONG_TEAMS], rand);
      const dieciseisavos = d16pool.slice(0, 28);
      const octavos = shuffle([...dieciseisavos], rand).slice(0, 14);
      const cuartos = shuffle([...octavos], rand).slice(0, 7);
      const semis = shuffle([...cuartos], rand).slice(0, 4);
      const finalTeams = shuffle([...semis], rand).slice(0, 2);

      const champ = pick(champions);
      const subcandidates = champions.filter((c) => c !== champ);
      const subcampeon = pick(subcandidates);
      const terceroCandidates = semis.filter((t) => !finalTeams.includes(t));
      const tercero = terceroCandidates.length > 0 ? terceroCandidates[0] : pick(subcandidates.filter((c) => c !== subcampeon));
      ...
      teams.push({
        ...
        groupOrderPicks,
        knockoutPicks: { dieciseisavos, octavos, cuartos, semis, finalTeams },
      });
      id++;
    }
  }
...
export const ACTIVITY = [
  ...
  { text: "Resultados de Jornada 1 actualizados", time: "Hace 1d" },
  { text: "Clasificación actualizada", time: "Hace 2d" },
];
...
export const SCORING = {
  ...
  posicionesFinales: { tercero: 20, subcampeon: 30, campeon: 50 },
  especiales: {

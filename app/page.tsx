...
import { Activity, BookOpen, ChevronRight, Shield, Swords, Trophy, TrendingUp, Zap } from "lucide-react";
...
export default function HomePage() {
  ...
  const quickLinks = [
    ...
    },
    {
      ...
      icon: TrendingUp,
      color: "#C1121F",
      cardStyle: { borderColor: "rgba(193,18,31,0.18)", background: "linear-gradient(135deg, rgba(193,18,31,0.08), rgba(255,255,255,0.02))" },
    },
    {
      href: "/mi-club",
      label: "Mi Club",
      sublabel: "Acceso privado",
      icon: Shield,
      color: "#6BBF78",
      cardStyle: {},
    },
      ...
      <div className="animate-fade-in text-center mb-7 pt-4">
        <div className="mb-1 flex items-center justify-center gap-3.5">
          ...
          <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl border border-gold/20 bg-bg-4">
            <Image src="/Logo_Porra_Mundial_2026.webp" alt="Peñita Mundial" width={44} height={44} className="object-contain" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4 animate-fade-in" style={{ animationDelay: "0.05s" }}>
        {quickLinks.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="card !p-3 text-center no-underline transition-all hover:!border-gold/20"
            style={item.cardStyle}
          >
            <item.icon size={20} className="mx-auto mb-1.5" style={{ color: item.color }} />
            <p className="text-[11px] font-semibold text-text-warm">{item.label}</p>
            <p className="text-[9px] text-text-muted">{item.sublabel}</p>
          </Link>
        ))}
      </div>

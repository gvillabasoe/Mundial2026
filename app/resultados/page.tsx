...
import { FIXTURES, GROUPS, PARTICIPANTS, type Fixture, type MatchPick, type Team } from "@/lib/data";
...
const PARTICIPANTS_BY_RANK = [...PARTICIPANTS].sort((a, b) => {
...
function buildPredictionDataset(match: MatchView, currentUserId: string): MatchPredictionDataset {
  ...
  if (match.stage === "group") {
    ...
    const rows = PARTICIPANTS_BY_RANK.map((team) => ({
      teamId: team.id,
  ...
  if (match.stage === "third-place") {
    return {
      rows: [],
      note: "La estructura actual del club no guarda picks por partido para el tercer puesto.",
  ...
  if (match.stage === "final") {
    ...
    return {
      rows,
      note: "En la final la estructura actual del club guarda finalistas y campeón, no un marcador exacto.",
      emptyTitle: "Sin pronósticos",
  ...
  if (!stageKey || typeof stageIndex !== "number") {
    return {
      rows: [],
      emptyTitle: "Sin detalle disponible",
      emptyText: "No se ha podido mapear este cruce con la estructura actual de la porra.",
    };
  }
  ...
  return {
    rows,
    note: "En eliminatorias la estructura actual del club guarda un clasificado por cruce; aquí se muestra ese pronóstico y no un marcador exacto.",
    emptyTitle: "Sin pronósticos",
    ...
    <>
      <div className="mx-auto max-w-[640px] px-4 pt-4">
        ...
        <p className="mb-3 text-[11px] text-text-muted">
          {filteredWorldCupCount} partidos del Mundial{testMatches.length > 0 ? ` + ${testMatches.length} test` : ""} · pulsa una card para ver los pronósticos del club
        </p>
      ...
      {selectedMatch ? (
        <MatchDetailModal match={selectedMatch} userId={user?.id || ""} onClose={() => setSelectedMatch(null)} />
      ) : null}
...
function MatchDetailModal({ match, userId, onClose }: { match: MatchView; userId: string; onClose: () => void }) {
  const detail = useMemo(() => buildPredictionDataset(match, userId), [match, userId]);
      ...
      >
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-text-muted">Detalle privado del club</p>
  ...
  return (
    <div className="fixed inset-0 z-[220] flex items-end justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-text-muted">Acceso privado del club</p>
            <h3 className="mt-1 font-display text-[22px] font-extrabold text-text-warm">{match.displayHomeTeam} vs {match.displayAwayTeam}</h3>
            <p className="mt-2 text-sm text-text-muted">Inicia sesión en Mi Club para ver los pronósticos del resto de participantes y los partidos con doble puntuación.</p>
          </div>
          <button onClick={onClose} className="rounded-xl border border-[rgb(var(--divider)/0.08)] bg-bg-2 p-2 text-text-muted transition-colors hover:text-text-primary">
            <X size={18} />
          </button>
